# Maticoo SDK Integration

1. [Introduction](#introduction)

2. [Integration Maticoo SDK](#integration)

    [2.1 Integrating the Maticoo SDK in to project](#step1)

    [2.2 Initialize the Maticoo SDK](#step2)
...
- Maticoo SDK supports Banner, Interstitial, Rewarded Video.
- Maticoo Android SDK supports Android API 15+.
- Please make sure you have added an app and at least one ad slot in Maticoo Platform.

## <a name="integration">2.Integration Maticoo SDK</a>  

### <a name="step1">2.1 Integrating the Maticoo SDK in to project</a>
...
* Detail of the different jars：
  ...
  | ------------------------ | ----------------------------------------------- | ------------ |
  | maticoo_base_xx.jar        | basic functions(banner\interstitial\native ads) | Y            |
  | maticoo_imageloader_xx.jar | imageloader functions                           | N            |
  | maticoo_appwall_xx.jar     | appwall ads functions                           | N            |
  | maticoo_video_xx.jar       | video ads functions                             | N            |
...
``` groovy
    dependencies {
        implementation files('libs/maticoo_base_xx.jar')
         //chrome custom tabs
...
```xml
	...
	<!-- Necessary -->
	<activity android:name="com.maticoo.kern.view.InnerWebViewActivity" />

	<provider
            android:authorities="${applicationId}.xxprovider"
            android:name="com.maticoo.kern.core.MaticooProvider"
...
**Please make sure to initialize Maticoo SDK before doing anything.**

```java
    MaticooSDK.initialize(context, "Your slotID");
```

**Set schema https**

```java
    MaticooSDK.setSchema(true);
```
...
```java
     ...
     */
     MaticooSDK.uploadConsent(this, true, "GDPR", new HttpRequester.Listener() {
            @Override
...
``` java
     //child-oriented
     MaticooSDK.setIsChildDirected(this, true);
```
Warning:
...
``` java
    #for sdk
    -keep public class com.maticoo.**{*;}
    -dontwarn com.maticoo.**
...
​	If you live in a country, such as China, which is forbidden google play, two prerequisites to get Maticoo ads: 
...
``` java
    /**
     * @param slotId     maticoo id
     * @param context    context
     * @param adListener callback listener 
     */
 	MaticooSDK.getNativeAd("Your slotID", context, new MyCTAdEventListener(){
        @Override
        public void onReceiveAdSucceed(GTNative result) {
            if (result == null) {
                return;
...
```java
 ...
     * @param adListener callback listener 
     */
    MaticooSDK.getNativeAdForCache("Your slotID",context,new MyCTAdEventListener() {
        @Override
...
``` java
 ...
     * @param adListener  callback listener 
     */
	MaticooSDK.getNativeAds(10, "Your slotID", getContext(), new MultiAdsEventListener() {
...
``` java
 ...
     * @param adListener        callback listener 
     */
	 MaticooSDK.getBannerAd(context, "Your slotID", adSize,new MyCTAdEventListener() {
         @Override
         public void onReceiveAdSucceed(GTNative result) {
             if (result != null) {
...
```xml
	<activity android:name="com.maticoo.kern.view.InterstitialActivity" />    
```
...
``` java
 ...
     * @param adListener        callback listener 
     */
    MaticooSDK.preloadInterstitialAd(context, "Your slotID",new MyCTAdEventListener() {
                    
        @Override
        public void onReceiveAdSucceed(GTNative result) {              
            if (result != null && result.isLoaded()) {
                Log.w(TAG, "onReceiveAdSucceed");
                if (MaticooSDK.isInterstitialAvailable(result)) {
            		MaticooSDK.showInterstitialAd(result);
        		}
            }
            super.onReceiveAdSucceed(result);
        }
...
MaticooSDK.preloadSplashAd(this, Config.slotIdSplash, new AdEventListener() {

    @Override
    public void onReceiveAdSucceed(GTNative result) {
        MaticooSDK.showSplashAd(Config.slotIdSplash, mSplashContainer);
    }
...
``` groovy
	dependencies {
        	compile files('libs/maticoo_base_xx.jar')
        	compile files('libs/maticoo_appwall_xx.jar')       // for appwall        
        	compile files('libs/maticoo_imageloader_xx.jar')   // for imageloader
	}
```
...
``` xml
    <activity
            android:name="com.maticoo.appwall.AppwallActivity"
...
``` java
   MaticooAppwall.preloadAppwall(context, "Your slotID");
```
...
``` java
    ...
    MaticooAppwall.setThemeColor(custimozeColor);
```

> Show Appwall.

``` java
     MaticooAppwall.showAppwall(context, "Your slotID");
```
...
``` groovy
	dependencies {
        	compile files('libs/maticoo_base_xx.jar')
        	compile files('libs/maticoo_video_xx.jar')
        	compile files('libs/maticoo_imageloader_xx.jar')
	}
```
...
``` xml    
<activity
	android:name="com.maticoo.video.view.RewardedVideoActivity"  android:configChanges="keyboard|keyboardHidden|orientation|screenLayout|uiMode|screenSize|smallestScreenSize" />
```
...
```java
	MaticooVideo.setUserId("custom_id");
```
...
``` java
 	MaticooVideo.preloadRewardedVideo(getContext(), Config.slotIdReward,
...
```java
    boolean available = MaticooVideo.isRewardedVideoAvailable(video);
```
...
```java
  MaticooVideo.showRewardedVideo(video, new RewardedVideoAdListener() {
                @Override
...
The Reward object contains both the Reward Name & Reward Amount of the SlotId as defined in your Maticoo SSP:
...
```java
   MaticooVideo.getNativeVideo(context, Config.slotIdNativeVideo, new VideoAdLoadListener(){
            @Override
